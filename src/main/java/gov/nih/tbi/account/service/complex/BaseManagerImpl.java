...
public class BaseManagerImpl implements BaseManager, Serializable {
  ...
  public Boolean hasRole(Account account, RoleType role) {
    ...
    authorities =
        roleHierarchy.getReachableGrantedAuthorities(AccountDetailService
            .getAuthorities(account.getAccountRoleList()));

    if (authorities.contains(new GrantedAuthorityImpl(role.getName()))) {
      return true;
    }
   ...
   * @inheritDoc
   */
  public void sendEmail(Account account, String subject, String messageText)
  ...
  public String getHash2(Account account) {
    ...
    if (account != null && account.getUserName() != null
        && !(account.getUserName().equals("anonymous"))) {
  ...
  /**
   * Escape the characters '_' '%' and '\' with '\' character for use in a
   * hibernate Restrictions.iLike() statement.
   * 
   * @param input
