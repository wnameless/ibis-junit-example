...
/**
 * This private class is an implementation of UserDetails, used by the spring
 * security. This is a wrapper for the Account class to conform to the Spring
 * Security's desired format. It allows for abstraction away from the Account
...
public class AccountUserDetails implements UserDetails {
  ...
  /**
   * Constructor requires an Account and a diseaseId to identify where the user
   * is registered
   * 
   * @param account
   * @param diseaseId
  ...
  /**
   * Returns the roles an account has in the form that Spring Security is
   * expecting
   */
  ...
  /**
   * This method returns a HEX representation of the bit array that is stored in
   * the database
   */
  ...
  /**
   * Checks to see if the user has logged in in the last 365 days, and if they
   * have a valid 'USER_ROLE'
   */
  public boolean isAccountNonExpired() {
    ...
    // A user must have an active "USER_ROLE" or "ADMIN_ROLE"
    {
      ...
      // A user must have logged in within the last ## of days (unless they are
      // an admin)
      if (account.getLastSuccessfulLogin() != null
          && (adminRole == null
              || !RoleStatus.ACTIVE.equals(adminRole.getRoleStatus()) || adminRole
                .isExpired())) {
      ...
      if (adminRole == null
          || !RoleStatus.ACTIVE.equals(adminRole.getRoleStatus())
          || adminRole.isExpired()) {
        // If the user doesn't have a user role, that is active and not expired
        // then return false
        if (userRole == null
            || !RoleStatus.ACTIVE.equals(userRole.getRoleStatus())
            || userRole.isExpired()) {
          return false;
        }
      }
  ...
  public boolean isAccountNonLocked() {
    ...
    if (account.getUnlockDate() != null
        && (new Date()).before(account.getUnlockDate())) {
      return false;
    }
  ...
  /**
   * This method is used by spring security to determine if the account should
  ...
  public boolean isEnabled() {
    ...
    if (!(AccountStatus.ACTIVE.equals(account.getAccountStatus()) || (AccountStatus.PENDING
