    ...
    if (account != null) {
      ...
      for (int i = 0; i < fields.length; i++) {
        ...
        if (!current.getName().equals("logger")
            && !ServiceConstants.STATIC_MANAGER.equals(current.getName())
  ...
  public void adapt(Account account, Boolean enforceStaticFields,
    ...
    for (int i = 0; i < fields.length; i++) {
      ...
      if (!current.getName().equals("logger")
        ...
        try {
          if (enforceStaticFields == false
              || current.getAnnotation(StaticField.class) == null) {
            ...
            for (Field userField : User.class.getDeclaredFields()) {
              if (userField.getName().equals(current.getName())) {
                setMethod =
                    user.getClass().getMethod(setMethodName, current.getType());
            ...
            for (Field accountField : Account.class.getDeclaredFields()) {
              if (accountField.getName().equals(current.getName())) {
                // Ignore admin note if namespace is 'accounts'.
                // Should also ignore during admin create but doesn't matter
                // since its null to begin
                // with anyway.
                setMethod =
                    account.getClass().getMethod(setMethodName,
                        current.getType());
    ...
    if (passwordString != null
        && !ServiceConstants.EMPTY_STRING.equals(passwordString)) {
      account.setPassword(accountManager.hashPassword(passwordString));
    }
    account.setUser(user);
  }
}
