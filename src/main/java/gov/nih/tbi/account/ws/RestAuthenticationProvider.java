...
public class RestAuthenticationProvider {
  ...
  /*************************************** HELPERS *************************************************/
  // THIS FUNCITONS MUST BE CALLED BEFORE AND AFTER EVERY WEB SERVICE CALL. EVEN
  // PUBLIC ONES!
  ...
  protected void ticketValid() {
    ...
    throw new IllegalArgumentException(
        "The proxy ticket for this restful web service call has already been used.");
  }
  ...
  /**
   * Builds a list of parameters from a list of variables and their arguments.
   * Will also encode the parameters in UTF-8.
   * 
   * @param variables
   * @param parameters
   * @return A list of parameters
   *         like...?variable1=parameter1&variable2=parameter2
   *         ...&variableN=parameterN
   * @throws UnsupportedEncodingException
   */
  public static String buildParameters(String[] variables, String[] parameters,
      String ticketProperty) throws UnsupportedEncodingException {

    StringBuilder builder = new StringBuilder();

    if (variables.length != parameters.length) {
      throw new RuntimeException(
          "Number of variables and parameters must be the same!");
    }
    ...
    for (int i = 0; i < variables.length; i++) {
      if (parameters[i] == null
          || ServiceConstants.EMPTY_STRING.equals(parameters[i])) {
        continue;
      }
      ...
      // variable=parameter
      builder.append(variables[i]).append("=")
          .append(encodeUrlParam(parameters[i]));
      parameterCount++;
    }

    if (ticketProperty != null
        && !ServiceConstants.EMPTY_STRING.equals(ticketProperty)) {
      if (parameterCount == 0) {
        builder.append("?");
      } else {
        builder.append("&");
